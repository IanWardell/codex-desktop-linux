...
function codexLinuxAccountSwitcherRead(){
  try{const value=JSON.parse(codexLinuxAccountSwitcherFs.readFileSync(codexLinuxAccountSwitcherConfigPath,"utf8"));if(!value||!Array.isArray(value.profiles))throw Error("invalid registry");for(const profile of value.profiles){if(!profile||!codexLinuxAccountSwitcherId(profile.id)||!codexLinuxAccountSwitcherId(profile.contextId||"default"))throw Error("invalid profile id in registry")}if(value.previousProfileId!=null&&!codexLinuxAccountSwitcherId(value.previousProfileId))throw Error("invalid previous profile id in registry");return value}catch(error){if(error?.code==="ENOENT")return {version:1,profiles:[]};throw error}
}
function codexLinuxAccountSwitcherWrite(value){
  ...
  const temporary=codexLinuxAccountSwitcherConfigPath+".tmp."+process.pid+"."+Date.now();
  codexLinuxAccountSwitcherFs.writeFileSync(temporary,JSON.stringify(value,null,2)+"\n",{encoding:"utf8",mode:384});
  const fd=codexLinuxAccountSwitcherFs.openSync(temporary,"r");try{codexLinuxAccountSwitcherFs.fsyncSync(fd)}finally{codexLinuxAccountSwitcherFs.closeSync(fd)}
  codexLinuxAccountSwitcherFs.renameSync(temporary,codexLinuxAccountSwitcherConfigPath);
}
function codexLinuxAccountSwitcherWithLock(mutator){codexLinuxAccountSwitcherFs.mkdirSync(codexLinuxAccountSwitcherConfigDir,{recursive:true,mode:448});const lock=codexLinuxAccountSwitcherConfigPath+".lock",deadline=Date.now()+5000;for(;;){try{codexLinuxAccountSwitcherFs.mkdirSync(lock,{recursive:false,mode:448});break}catch(error){if(error?.code!=="EEXIST")throw error;if(Date.now()>=deadline)throw Error("Account-switcher registry is busy");try{const owner=Number(codexLinuxAccountSwitcherFs.readFileSync(codexLinuxAccountSwitcherPath.join(lock,"pid"),"utf8"));if(Number.isInteger(owner)&&owner>0){try{process.kill(owner,0)}catch{codexLinuxAccountSwitcherFs.rmSync(lock,{recursive:true,force:true});continue}}}catch{}Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,10)}}try{codexLinuxAccountSwitcherFs.writeFileSync(codexLinuxAccountSwitcherPath.join(lock,"pid"),String(process.pid),{mode:384});const value=codexLinuxAccountSwitcherRead(),result=mutator(value);codexLinuxAccountSwitcherWrite(value);return result}finally{codexLinuxAccountSwitcherFs.rmSync(lock,{recursive:true,force:true})}}
...
function codexLinuxAccountSwitcherWriteHandoff(source,target,removeId){codexLinuxAccountSwitcherFs.mkdirSync(codexLinuxAccountSwitcherConfigDir,{recursive:true,mode:448});const temporary=codexLinuxAccountSwitcherHandoffPath+".tmp."+process.pid;const lines=["version=1","phase=requested","nonce="+Date.now().toString(36)+"-"+process.pid,"from_id="+(source?.id||"default"),"from_mode="+(source?.contextMode||"isolated"),"from_context="+(source?.contextId||"default"),"target_id="+target.id,"target_mode="+(target.contextMode||"isolated"),"target_context="+(target.contextId||"default")];if(removeId)lines.push("remove_id="+removeId);codexLinuxAccountSwitcherFs.writeFileSync(temporary,lines.join("\n")+"\n",{encoding:"utf8",mode:384});codexLinuxAccountSwitcherFs.renameSync(temporary,codexLinuxAccountSwitcherHandoffPath)}
...
function codexLinuxAccountSwitcherPublic(profile,details={}){const signedIn=codexLinuxAccountSwitcherHasAuth(profile);return{id:profile.id,name:profile.name,login:details.email||profile.email||null,signedIn,removable:profile.id!=="default"&&!signedIn,usagePercent:Number.isFinite(details.usagePercent)?details.usagePercent:null,usageUpdatedAt:details.usageUpdatedAt||profile.usageUpdatedAt||null,contextMode:profile.contextMode||"isolated",contextId:profile.contextId||"default"}}
...
function codexLinuxAccountSwitcherFinalizeRemoval(){let id;try{id=codexLinuxAccountSwitcherId(codexLinuxAccountSwitcherFs.readFileSync(codexLinuxAccountSwitcherRemoveCompletePath,"utf8").trim())}catch(error){if(error?.code!=="ENOENT")console.warn("WARN: account-switcher removal recovery failed: "+error.message);return}if(!id||id==="default"){console.warn("WARN: account-switcher ignored invalid removal completion");return}try{codexLinuxAccountSwitcherWithLock((latest)=>{latest.profiles=latest.profiles.filter((entry)=>entry&&entry.id!==id);if(latest.previousProfileId===id)delete latest.previousProfileId});codexLinuxAccountSwitcherFs.rmSync(codexLinuxAccountSwitcherRemoveCompletePath,{force:true})}catch(error){console.warn("WARN: account-switcher could not finalize profile removal: "+error.message)}}
function codexLinuxAccountSwitcherArmRemovalRecovery(){codexLinuxAccountSwitcherFinalizeRemoval();codexLinuxAccountSwitcherFs.watchFile(codexLinuxAccountSwitcherRemoveCompletePath,{interval:250},codexLinuxAccountSwitcherFinalizeRemoval);l.app?.once?.("before-quit",()=>codexLinuxAccountSwitcherFs.unwatchFile(codexLinuxAccountSwitcherRemoveCompletePath,codexLinuxAccountSwitcherFinalizeRemoval))}
...
function codexLinuxAccountSwitcherRelaunch(profile,source,removeId){
  codexLinuxAccountSwitcherWriteActive(profile);
  codexLinuxAccountSwitcherWriteHandoff(source,profile,removeId);
  try{l.app.quit()}catch(error){throw Error("Account handoff could not begin: "+error.message)}
...
if(l.app?.whenReady)l.app.whenReady().then(codexLinuxAccountSwitcherArmLogoutFallback).catch(()=>{});
l.ipcMain.handle(codexLinuxAccountSwitcherIpc,async(codexLinuxAccountSwitcherEvent,request={})=>{
  ...
  if(action==="refresh"){
    const snapshot=registry.profiles.map((profile)=>({id:profile.id,details:codexLinuxAccountSwitcherDetails(profile)})),details=await Promise.all(snapshot.map((entry)=>entry.details)),now=new Date().toISOString();
    return codexLinuxAccountSwitcherWithLock((latest)=>{codexLinuxAccountSwitcherDefault(latest);const byId=new Map(snapshot.map((entry,index)=>[entry.id,details[index]]));for(const profile of latest.profiles){const value=byId.get(profile.id);if(!value)continue;if(value.email)profile.email=value.email;if(value.usagePercent!=null){profile.usagePercent=value.usagePercent;profile.usageUpdatedAt=value.usageUpdatedAt||now}}return{profiles:latest.profiles.map((profile)=>codexLinuxAccountSwitcherPublic(profile)),activeProfileId:process.env.CODEX_LINUX_ACCOUNT_SWITCHER_PROFILE||"default",keepLocalProjectsThreads:codexLinuxAccountSwitcherKeepLocalProjectsThreads(latest)}})
  }
  if(action==="set-settings"){
    return codexLinuxAccountSwitcherWithLock((latest)=>{const active=codexLinuxAccountSwitcherSetKeepLocalProjectsThreads(latest,request.keepLocalProjectsThreads===true);if(active)codexLinuxAccountSwitcherWriteActive(active);return{ok:true,keepLocalProjectsThreads:codexLinuxAccountSwitcherKeepLocalProjectsThreads(latest)}});
  }
  if(action==="create"){
